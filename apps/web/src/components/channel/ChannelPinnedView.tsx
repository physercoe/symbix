  ...
  return (
    <ScrollArea className="flex-1 p-6">
      <div className="mx-auto max-w-2xl">
        <h2 className="text-lg font-semibold mb-4">{t('pinned.title' as any)}</h2>
        {!pins?.length ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="text-sm">{t('pinned.noPinned' as any)}</p>
            <p className="text-xs mt-1">{t('pinned.hintPin' as any)}</p>
          </div>
        ) : (
            ...
            {pins.map((pin) => (
              <div key={pin.id} className="rounded-lg border p-4 group hover:bg-accent/20 transition-colors">
                <div className="flex items-center justify-between mb-2">
                  ...
                  >
                    {t('pinned.unpin' as any)}
                  </button>
                </div>
                {pin.content ? (
                  <Markdown content={pin.content} className="text-sm" />
                ) : (
                  <p className="text-sm text-muted-foreground italic">{t('pinned.mediaAttachment' as any)}</p>
                )}
              </div>
            ))}
          </div>
