  ...
  return (
    <ScrollArea className="flex-1 p-6">
      <div className="mx-auto max-w-3xl">
        <h2 className="text-lg font-semibold mb-4">{t('files.title' as any)}</h2>

        {/* Shared in chat */}
        <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3">{t('files.sharedInChat' as any)}</h3>
        {fileMessages.length === 0 ? (
          <p className="text-sm text-muted-foreground mb-6">{t('files.noFilesInChat' as any)}</p>
        ) : (
        ...
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('files.savedFiles' as any)}</h3>
          <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setShowAdd(!showAdd)}>+ {t('files.addFile' as any)}</Button>
        </div>
        ...
        {showAdd && (
          <form onSubmit={(e) => { e.preventDefault(); if (title.trim()) create.mutate({ channelId, type: 'file', title: title.trim(), url: url.trim() || undefined }); }}
            ...
            <input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t('files.fileName' as any)}
            ...
            <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder={t('files.urlOptional' as any)}
              className="w-full rounded border border-input bg-background px-3 py-1.5 text-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
            <div className="flex gap-2">
              <Button type="submit" size="sm" className="h-7 text-xs" disabled={create.isPending}>{t('common.add' as any)}</Button>
              <Button type="button" variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setShowAdd(false)}>{t('common.cancel' as any)}</Button>
            </div>
          </form>
        )}

        {!savedFiles?.length && !showAdd && <p className="text-sm text-muted-foreground">{t('files.noSavedFiles' as any)}</p>}
        ...
        <div className="space-y-2">
          {savedFiles?.map((item) => (
            <div key={item.id} className="flex items-center gap-3 rounded-lg border p-3 group hover:bg-accent/20 transition-colors">
              ...
              <button type="button" onClick={() => { if (confirm(t('files.removeConfirm' as any))) remove.mutate({ id: item.id }); }}
