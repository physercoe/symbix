...
const STATUS_LABEL_KEYS: Record<string, string> = { open: 'tasks.status.open', in_progress: 'tasks.status.in_progress', done: 'tasks.status.done' };
    ...
    <ScrollArea className="flex-1 p-6">
      <div className="mx-auto max-w-3xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">{t('tasks.title' as any)}</h2>
          <span className="text-xs text-muted-foreground">{tasks?.length ?? 0} {t('tasks.total' as any)}</span>
        </div>
        ...
        >
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t('tasks.addPlaceholder' as any)}
          ...
          <Button type="submit" size="sm" disabled={!title.trim() || create.isPending}>{t('common.add' as any)}</Button>
        </form>
          ...
          {STATUS_ORDER.map((status) => (
            <div key={status}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-medium">{t(STATUS_LABEL_KEYS[status] as any)}</h3>
                <span className="text-xs text-muted-foreground ml-auto">{grouped[status].length}</span>
              </div>
              <div className="space-y-2">
                {grouped[status].length === 0 && (
                  <p className="text-xs text-muted-foreground py-4 text-center border border-dashed rounded-lg">{t('tasks.noTasks' as any)}</p>
                )}
                  ...
                  <div key={task.id} className="rounded-lg border p-3 group hover:bg-accent/20 transition-colors">
                    <div className="flex items-start gap-2">
                      <button type="button" onClick={() => update.mutate({ id: task.id, status: cycleStatus(task.status) })}
                        className="mt-0.5 shrink-0" title={`${t('tasks.moveTo' as any)} ${t(STATUS_LABEL_KEYS[cycleStatus(task.status)] as any)}`}>
                      ...
                      <button type="button" onClick={() => { if (confirm(t('tasks.deleteConfirm' as any))) remove.mutate({ id: task.id }); }}
