...
function PatternsIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><polyline points="16 18 22 12 16 6" /><polyline points="8 6 2 12 8 18" /></svg>;
}
function BookmarkIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" /></svg>;
}
...
function BoxIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /></svg>;
}
...
export function TeamSidebar() {
  ...
  // teamSlug comes from URL params when on /t/[teamSlug]/* pages,
  // falls back to localStorage when on /toolkit/* pages
  const paramTeamSlug = params.teamSlug as string | undefined;
  const [storedTeamSlug, setStoredTeamSlug] = useState<string | null>(null);

  useEffect(() => {
    const stored = localStorage.getItem('symbix:lastTeamSlug');
    if (stored) setStoredTeamSlug(stored);
  }, []);

  useEffect(() => {
    if (paramTeamSlug) {
      localStorage.setItem('symbix:lastTeamSlug', paramTeamSlug);
      setStoredTeamSlug(paramTeamSlug);
    }
  }, [paramTeamSlug]);

  const teamSlug = paramTeamSlug ?? storedTeamSlug;
  ...
  // Tab sync with URL
  const isToolkitPath = pathname.startsWith('/toolkit');
  const [tab, setTab] = useState<SidebarTab>(isToolkitPath ? 'toolkit' : 'team');

  useEffect(() => {
    if (isToolkitPath && tab !== 'toolkit') setTab('toolkit');
    else if (!isToolkitPath && tab !== 'team') setTab('team');
  }, [pathname, isToolkitPath]);

  const handleTeamTab = useCallback(() => {
    setTab('team');
    if (isToolkitPath && teamSlug) {
      router.push(`/t/${teamSlug}`);
    }
  }, [isToolkitPath, teamSlug, router]);
  ...
  const base = teamSlug ? `/t/${teamSlug}` : '/';
    ...
    <div className="flex h-full w-64 flex-col bg-sidebar text-sidebar-foreground border-r">
      ...
      <ScrollArea className="flex-1 px-2 py-2">
        {tab === 'team' ? (
          ...
          /* Toolkit tab */
          <div className="space-y-0.5">
            <NavLink href="/toolkit/specs" label="Specs" icon={<SpecsIcon />} pathname={pathname} />
            <NavLink href="/toolkit/patterns" label="Patterns" icon={<PatternsIcon />} pathname={pathname} />
            <NavLink href="/toolkit/references" label="References" icon={<BookmarkIcon />} pathname={pathname} />
            <NavLink href="/toolkit/insights" label="Insights" icon={<LightbulbIcon />} pathname={pathname} />
            <NavLink href="/toolkit/assets" label="Assets" icon={<BoxIcon />} pathname={pathname} />
            <NavLink href="/toolkit/machines" label="Machines" icon={<MonitorIcon />} pathname={pathname} />
          </div>
        )}
      </ScrollArea>
