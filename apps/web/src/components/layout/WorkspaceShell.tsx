...
export function WorkspaceShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function connect() {
      ...
      if (token && mounted) wsManager.connect(token);
    }

    connect();

    const onNewMessage = (data: unknown) => {
      const d = data as { message: { id: string; channelId: string; senderType: string; senderId: string; content: string | null; contentType: string; mediaUrl: string | null; metadata: Record<string, unknown> | null; parentId: string | null; createdAt: string } };
      if (d.message) {
        if (d.message.senderType === 'agent') clearStreaming(d.message.senderId);
